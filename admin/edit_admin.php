...
if(isset($_POST['edit'])){
    ...
    $pass = $_POST['password'];
    $nama = $_POST['nama'];
    $update = $conn->query("UPDATE user SET username='$username', password='$pass', nama='$nama' WHERE id='$id' AND class='admin' ");
    if($update){
...
<div class="card">
  ...
  <div class="card-body">
    ...
    <?php 
        $id = $_GET['id'];
        $dapat = $conn->query("SELECT * FROM user WHERE id='$id' AND class='admin'");
        if($dapat->num_rows>0){
